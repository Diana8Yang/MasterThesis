...
#### hmm
# list of xlim values CM
{
  x_list_C <- cbind(seq(from=0.4, to=1, length.out = 10^4),
                    seq(from=0, to=400, length.out = 10^4),
                    seq(from=-10, to=5, length.out = 10^4),
                    seq(from=0, to=1, length.out = 10^4),
                    seq(from=-1.4, to=0, length.out = 10^4),
                    seq(from=0, to=2000, length.out = 10^4),
                    seq(from=0, to=0.01, length.out = 10^4),
                    
                    seq(from=0, to=0.5, length.out = 10^4),
                    seq(from=0, to=15, length.out = 10^4),
                    seq(from=0, to=0.5, length.out = 10^4),
                    seq(from=0, to=8, length.out = 10^4),
                    
                    seq(from=-15, to=15, length.out = 10^4),
                    seq(from=-15, to=15, length.out = 10^4),
                    seq(from=-20, to=10, length.out = 10^4),
                    seq(from=-10, to=10, length.out = 10^4),
                    seq(from=-10, to=10, length.out = 10^4),
                    seq(from=-10, to=10, length.out = 10^4),
                    
                    seq(from=-4, to=4, length.out = 10^4),
                    seq(from=-4, to=4, length.out = 10^4),
                    seq(from=-4, to=4, length.out = 10^4),
                    seq(from=-4, to=4, length.out = 10^4),
                    
                    seq(from=0, to=8, length.out = 10^4),
                    seq(from=0, to=10, length.out = 10^4),
                    seq(from=-2, to=10, length.out = 10^4),
                    seq(from=0, to=10, length.out = 10^4),
                    seq(from=-10, to=20, length.out = 10^4),
                    
                    seq(from=-6, to=0, length.out = 10^4),
                    seq(from=-7, to=0, length.out = 10^4),
                    seq(from=-12, to=0, length.out = 10^4),   
                    seq(from=-15, to=5, length.out = 10^4),
                    seq(from=-15, to=10, length.out = 10^4))
}

# list of y values CM
{
  plot_iprior_C <- cbind(dbeta(seq(from=0.4, to=1, length.out = 10^4), iprior.pw$estimate[1], iprior.pw$estimate[2]),
                         dlnorm(seq(from=0, to=400, length.out = 10^4), iprior.taue$estimate[1], iprior.taue$estimate[2]),
                         dnorm(seq(from=-10, to=5, length.out = 10^4), iprior.logitpb$estimate[1], iprior.logitpb$estimate[2]),
                         dnorm(seq(from=0, to=1, length.out = 10^4), iprior.e_1$estimate[1], iprior.e_1$estimate[2]),
                         dnorm(seq(from=-1.4, to=0, length.out = 10^4), iprior.mu_zero$estimate[1], iprior.mu_zero$estimate[2]),
                         dgamma(seq(from=0, to=2000, length.out = 10^4), iprior.tauw$estimate[1], iprior.tauw$estimate[2]),
                         dgamma(seq(from=0, to=0.01, length.out = 10^4), iprior.sdb$estimate[1], rate=iprior.sdb$estimate[2]),
                         # CPM Christensen
                         dbeta(seq(from=0, to=0.5, length.out = 10^4), iprior.t.t_ce$estimate[1], iprior.t.t_ce$estimate[2]),
                         dgamma(seq(from=0, to=15, length.out = 10^4), iprior.s.t_ce$estimate[1], rate=iprior.s.t_ce$estimate[2]),
                         dbeta(seq(from=0, to=0.5, length.out = 10^4), iprior.t.t_es$estimate[1], iprior.t.t_es$estimate[2]),
                         dgamma(seq(from=0, to=8, length.out = 10^4), iprior.s.t_es$estimate[1], rate=iprior.s.t_es$estimate[2]),
                         # Teunis
                         dnorm(seq(from=-15, to=15, length.out = 10^4), iprior.w[1,1], iprior.w[1, 2]),
                         dnorm(seq(from=-15, to=15, length.out = 10^4), iprior.w[2,1], iprior.w[2, 2]),
                         dnorm(seq(from=-20, to=10, length.out = 10^4), iprior.w[3,1], iprior.w[3, 2]),
                         dnorm(seq(from=-10, to=10, length.out = 10^4), iprior.w[1,3], iprior.w[1, 4]),
                         dnorm(seq(from=-10, to=10, length.out = 10^4), iprior.w[2,3], iprior.w[2, 4]),
                         dnorm(seq(from=-10, to=10, length.out = 10^4), iprior.w[3,3], iprior.w[3, 4]),
                         
                         dnorm(seq(from=-4, to=4, length.out = 10^4), iprior.z1[9, 3], iprior.z1[9, 4]),
                         dnorm(seq(from=-4, to=4, length.out = 10^4), iprior.z1[9, 5], iprior.z1[9, 6]),
                         dnorm(seq(from=-4, to=4, length.out = 10^4), iprior.z2[9, 3], iprior.z2[9, 4]),
                         dnorm(seq(from=-4, to=4, length.out = 10^4), iprior.z2[9, 5], iprior.z2[9, 6]),
                         
                         dnorm(seq(from=0, to=8, length.out = 10^4), iprior.logconc1$estimate[1], iprior.logconc1$estimate[2]),
                         dnorm(seq(from=0, to=10, length.out = 10^4), iprior.logconc2$estimate[1], iprior.logconc2$estimate[2]),
                         dnorm(seq(from=-2, to=10, length.out = 10^4), iprior.logconc3$estimate[1], iprior.logconc3$estimate[2]),
                         dnorm(seq(from=0, to=10, length.out = 10^4), iprior.logconc4$estimate[1], iprior.logconc4$estimate[2]),
                         dnorm(seq(from=-10, to=20, length.out = 10^4), iprior.logconc5$estimate[1], iprior.logconc5$estimate[2]),
                         
                         dnorm(seq(from=-6, to=0, length.out = 10^4), iprior.ltp0_1$estimate[1], iprior.ltp0_1$estimate[2]),
                         dnorm(seq(from=-7, to=0, length.out = 10^4), iprior.ltp0_2$estimate[1], iprior.ltp0_2$estimate[2]),
                         dnorm(seq(from=-12, to=0, length.out = 10^4), iprior.ltp0_3$estimate[1], iprior.ltp0_3$estimate[2]),
                         dnorm(seq(from=-15, to=5, length.out = 10^4), iprior.ltp0_4$estimate[1], iprior.ltp0_4$estimate[2]),
                         dnorm(seq(from=-15, to=10, length.out = 10^4), iprior.ltp0_5$estimate[1], iprior.ltp0_5$estimate[2]))
}                    

# Plot the posterior CM
{
  #par(mfcol=c(1,1))
  par(mfrow=c(3,3), mar = c(1, 1, 4, 1), oma=c(1, 1, 3, 1))
  for(i in 1:7){
    #i = 4
    minhist_C <- min(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    maxhist_C <- max(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    hist(posterior_prior_ks_C[,i],  col='skyblue',border=F, xlim=c(minhist_C, maxhist_C),
         xlab = "", main = posterior_names_C[i], freq = F)                     
    hist(posterior_iprior_ks_C[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
    lines(x_list_C[,i], plot_iprior_C[,i], type="l")
  }
  mtext("Prevalence model", line = -0.5,outer = TRUE, cex = 1.5)
  
  #plot(x_list[,6], plot_iprior[,6])
  
  # Christensen CPM
  par(mfrow=c(2,2), mar = c(1, 1, 4, 1), oma=c(1, 1, 3, 1))
  for(i in 8:11){
    #i = 1
    minhist <- min(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    maxhist <- max(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    hist(posterior_prior_ks_C[,i],  col='skyblue',border=F, xlim=c(minhist, maxhist),
         xlab = "", main = posterior_names_C[i], freq = F)                     
    hist(posterior_iprior_ks_C[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
    lines(x_list_C[,i], plot_iprior_C[,i], type="l")
  }
  mtext("Christensen CPM", line = -0.5,outer = TRUE, cex = 1.5)
  
  # Dose-response
  par(mfrow=c(4,3), mar = c(1, 1, 4, 1), oma=c(1, 1, 3, 1))
  #for(i in 12:31){
  for(i in c(12:21, 26, 31)){ # parameters for prediction only
    #i = 1
    minhist <- min(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    maxhist <- max(posterior_prior_ks_C[,i], posterior_iprior_ks_C[,i])
    hist(posterior_prior_ks_C[,i],  col='skyblue',border=F, xlim=c(minhist, maxhist),
    ...
    hist(posterior_iprior_ks_C[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
    lines(x_list_C[,i], plot_iprior_C[,i], type="l")
    mtext("Dose-response model", line = -0.5,outer = TRUE, cex = 1.5)
  }
}

# Check the parameters
{
  plot(density(t_cemat))
  hist(t_ce,probability = TRUE,xlim = c(0,1))
  lines(density(t_cemat), col= "red")
  hist(t_es,probability = TRUE,xlim = c(0,1))
  lines(density(t_esmat), col = "red")
}

## Mylius
# Plot the informed priors
{
  par(mfrow=c(3,2))
  plot(density(rbeta(1000, iprior.t.t_ch$estimate[1], iprior.t.t_ch$estimate[2])))
  plot(density(rgamma(1000, iprior.s.t_ch$estimate[1], iprior.s.t_ch$estimate[2])))
  plot(density(rbeta(1000, iprior.t.t_cb$estimate[1], iprior.t.t_cb$estimate[2])))
  plot(density(rgamma(1000, iprior.s.t_cb$estimate[1], iprior.s.t_cb$estimate[2])))
  plot(density(rnorm(1000, iprior.mu.t_bs$estimate[1], iprior.mu.t_bs$estimate[2])))
  plot(density(rgamma(1000, iprior.precision.t_bs$estimate[1], iprior.precision.t_bs$estimate[2])))
  
}

# Check the transfer rates posterior vs data
{
  hist(t_ch,probability = TRUE,xlim = c(0,1))
  lines(density(t_chmat), col= "red")
  hist(t_cb,probability = TRUE,xlim = c(0,1))
  lines(density(t_cbmat), col = "red")
  hist(-log10(t_bs),probability = TRUE)
  lines(density(neg_log_t_bs_mat), col = "red")

}

# combined christensen 
{
# Kolomonoff-Smirnof test of the posterior samples
{
  # posterior based on informed prior (combined model)
  posterior_iprior_ks_C <- cbind(pw_C,taue_C,logitpb_1_C,
                                 e_1_C, mu_zero_C, tauw_C,
                                 sdb_t_C,
                                 prior.t.t_cemat_C, prior.s.t_cemat_C,
                                 prior.t.t_esmat_C, prior.s.t_esmat_C,
                                 mu.w_mat_C, z1_mat_C[,18],z1_mat_C[,27],
                                 z2_mat_C[,18],  z2_mat_C[,27],
                                 logconc_mat_C, ltp0_mat_C)        
  
  # posterior based on prior (individual models)
  posterior_prior_ks_C <- cbind(pw,taue,logitpb_1,
                                e_1, mu_zero, tauw,
                                sdb_t,
                                prior.t.t_cemat, prior.s.t_cemat,
                                prior.t.t_esmat, prior.s.t_esmat,
                                mu.w_mat, z1_mat[,18],z1_mat[,27],
                                z2_mat[,18],  z2_mat[,27],
                                logconc_mat, ltp0_mat)
  
  # Kolgomorov smirnoff test of the two samples above
  ks_test_result_C <- c()
  for(i in 1:31){
    ks_test_result_C[i] <- ks.test(posterior_iprior_ks_C[,i], posterior_prior_ks_C[,i])$p.value
  }
}

# Plot of the posteriors and informed priors  
{      
  posterior_names_C <- c("pw","taue","logitpb_1", "e_1", "mu_zero", "tauw",
                         "sdb_t",
                         "prior.t.t_cemat", "prior.s.t_cemat",
                         "prior.t.t_esmat", "prior.s.t_esmat",
                         "mu.w11", "mu.w21", "mu.w31", "mu.w12", "mu.w22", "mu.w32",
                         "z1_92","z1_93",
                         "z2_92",  "z2_93",
                         "logconc1", "logconc2", "logconc3", "logconc4", "logconc5",
                         "ltp0_1", "ltp0_2", "ltp0_3", "ltp0_4", "ltp0_5")
  
  colnames(posterior_prior_ks_C) <- posterior_names_C
  ...
  for(i in 1:7){
    ...
    hist(posterior_prior_ks_C[,i],  col='skyblue',border=F, #xlim=c(minhist_C, maxhist_C),
         xlab = "", main = posterior_names_C[i], freq = F)                     
    hist(posterior_iprior_ks_C[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
    plot(x_list_C[,i], plot_iprior_C[,i], type="l")
  }
  mtext("Prevalence model, C", line = -0.5,outer = TRUE, cex = 1.5)
  ...
  for(i in c(12:21, 26, 31)){ # parameters for prediction only
    ...
    hist(posterior_iprior_ks_C[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
    lines(x_list_C[,i], plot_iprior_C[,i], type="l")
    mtext("Dose-response model, C", line = -0.5,outer = TRUE, cex = 1.5)
  }
}
}   
...
{ 
  # Plot of the posterior
  {    
    posterior_names_M <- c("pw","taue","logitpb_1", "e_1", "mu_zero", "tauw",
                           "sdb_t",
                           "prior.t.t_chmat", "prior.s.t_chmat",
                           "prior.t.t_cbmat", "prior.s.t_cbmat",
                           "prior.mu.t_bsmat", "prior.precision.t_bsmat",
                           "mu.w11", "mu.w21", "mu.w31", "mu.w12", "mu.w22", "mu.w32",
                           "z1_92","z1_93",
                           "z2_92",  "z2_93",
                           "logconc1", "logconc2", "logconc3", "logconc4", "logconc5",
    ...
    # xlist for plotting
    {
      x_list_M <- cbind(seq(from=0.4, to=1, length.out = 10^4),
                        seq(from=0, to=400, length.out = 10^4),
                        seq(from=-10, to=5, length.out = 10^4),
                        seq(from=0, to=1, length.out = 10^4),
                        seq(from=-1.4, to=0, length.out = 10^4),
                        ...
                        seq(from=0, to=0.5, length.out = 10^4),
                        seq(from=0, to=15, length.out = 10^4),
                        seq(from=-3, to=3, length.out = 10^4),
                        seq(from=0, to=1, length.out = 10^4),
    ...
    {
      plot_iprior_M <- cbind(dbeta(seq(from=0.4, to=1, length.out = 10^4), iprior.pw$estimate[1], iprior.pw$estimate[2]),
                             dlnorm(seq(from=0, to=400, length.out = 10^4), iprior.taue$estimate[1], iprior.taue$estimate[2]),
                             dnorm(seq(from=-10, to=5, length.out = 10^4), iprior.logitpb$estimate[1], iprior.logitpb$estimate[2]),
                             dnorm(seq(from=0, to=1, length.out = 10^4), iprior.e_1$estimate[1], iprior.e_1$estimate[2]),
                             dnorm(seq(from=-1.4, to=0, length.out = 10^4), iprior.mu_zero$estimate[1], iprior.mu_zero$estimate[2]),
                             ...
                             dgamma(seq(from=0, to=0.01, length.out = 10^4), iprior.sdb$estimate[1], rate=iprior.sdb$estimate[2]),
                             # CPM Mylius
                             dbeta(seq(from=0, to=0.5, length.out = 10^4), iprior.t.t_ch$estimate[1], iprior.t.t_ch$estimate[2]),
                             dgamma(seq(from=0, to=15, length.out = 10^4), iprior.s.t_ch$estimate[1], rate=iprior.s.t_ch$estimate[2]),
                             dbeta(seq(from=0, to=0.5, length.out = 10^4), iprior.t.t_cb$estimate[1], iprior.t.t_cb$estimate[2]),
                             dgamma(seq(from=0, to=15, length.out = 10^4), iprior.s.t_cb$estimate[1], rate=iprior.s.t_cb$estimate[2]),
                             dnorm(seq(from=-3, to=3, length.out = 10^4), iprior.mu.t_bs$estimate[1], iprior.mu.t_bs$estimate[2]),
                             dgamma(seq(from=0, to=1, length.out = 10^4), iprior.precision.t_bs$estimate[1], rate=iprior.precision.t_bs$estimate[2]),
                             # Teunis
                             dnorm(seq(from=-15, to=15, length.out = 10^4), iprior.w[1,1], iprior.w[1, 2]),
                             dnorm(seq(from=-15, to=15, length.out = 10^4), iprior.w[2,1], iprior.w[2, 2]),
                             dnorm(seq(from=-20, to=10, length.out = 10^4), iprior.w[3,1], iprior.w[3, 2]),
    ...
    for(i in 1:7){
      #i = 6
      minhist_M <- min(posterior_prior_ks_M[,i], posterior_iprior_ks_M[,i])
      maxhist_M <- max(posterior_prior_ks_M[,i], posterior_iprior_ks_M[,i])
      hist(posterior_prior_ks_M[,i],  col='skyblue',border=F, xlim=c(minhist_M, maxhist_M),
           xlab = "", main = posterior_names_M[i], freq = F)                     
      hist(posterior_iprior_ks_M[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
      lines(x_list_M[,i], plot_iprior_M[,i], type="l")
    }
    mtext("Prevalence model, M", line = -0.5,outer = TRUE, cex = 1.5)
    ...
    mtext("Mylius CPM", line = -0.5,outer = TRUE, cex = 1.5)
    
    # Dose-response
    par(mfrow=c(4,3), mar = c(1, 1, 4, 1))
    #for(i in 12:31){
    for(i in c(14:23, 28, 33)){ # parameters for prediction only
      #i = 1
      minhist_M <- min(posterior_prior_ks_M[,i], posterior_iprior_ks_M[,i])
      maxhist_M <- max(posterior_prior_ks_M[,i], posterior_iprior_ks_M[,i])
      hist(posterior_prior_ks_M[,i],  col='skyblue',border=F, xlim=c(minhist_M, maxhist_M),
      ...
      hist(posterior_iprior_ks_M[,i], col=scales::alpha('red',.5),border=F, freq=F, add=T)  
      lines(x_list_M[,i], plot_iprior_M[,i], type="l")
      mtext("Dose-response model, M", line = -0.5,outer = TRUE, cex = 1.5)
    }
  }
}
